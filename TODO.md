...
- [x] P5-1 FigureDetailDialog 接收真实 FigureItem 数据，展示标题、描述、相册名、标签
- [x] P5-2 单图收藏：展示 After 大图预览（`object-contain` 适应容器，含加载态/错误态）
- [x] P5-3 双图收藏：展示 Tabs（滑杆对比 / 橡皮擦对比 / 原图 / AI 图），具体对比组件在 Phase 6-7 实现，先用占位或单独查看

### 5.2 星标收藏

- [x] P5-4 详情弹窗中实现星标按钮，点击调用 `toggleFavorite` 并更新 UI
- [-] P5-5 FigureCard 上的星标按钮同步调用 `toggleFavorite`（延后/调整为详情内星标——沉浸式图片墙不在卡片显示星标）
- [x] P5-6 切换星标后收藏墙实时刷新（如果当前在"收藏"筛选下，取消星标后该项从列表消失）

### 5.3 删除收藏项

- [x] P5-7 详情弹窗中添加删除按钮
- [x] P5-8 实现删除确认 AlertDialog（"确定要删除这个收藏项吗？此操作将删除数据库记录和应用内保存的图片副本，无法恢复。"）
- [x] P5-9 确认后执行删除：删除数据库记录（figures + figure_images + figure_tags 级联）
- [x] P5-10 确认后执行删除：删除 `images/{figureId}/` 目录及其中的图片文件
- [x] P5-11 删除完成后关闭详情弹窗并刷新收藏墙
- [x] P5-12 处理删除失败情况（文件删除失败时仍应删除数据库记录，避免幽灵数据；显示 toast 提示残留）
...
- [x] P7.5-AC 确认：编辑预填正确、修改信息/替换图片/添加移除 Before 都正常、保存后详情与收藏墙同步、构建通过

---

## Phase 8 + 9：相册标签完善 & 搜索筛选排序优化（合并）

> 目标：完善相册/标签管理体验 + 提升收藏墙检索与浏览效率。

### 8.1 相册管理

- [x] P8-1 实现 `AlbumFormDialog`（输入名称 + 可选描述）
- [x] P8-2 在 FigureFormDialog 中添加相册选择下拉 + "新建相册" 快捷入口
- [x] P8-3 新建相册后自动选中该相册，刷新本地 localAlbums

### 8.2 标签输入优化

- [x] P8-5 实现 `TagInput` 组件：Enter/逗号创建标签 Badge + 自动补全建议
- [x] P8-6 已选标签 Badge 点击可删除；新标签虚线边框标识
- [x] P8-7 标签自动创建逻辑：`TagInput` 输入未知标签时生成临时 `{ id, name }`，保存时 `getOrCreateTag`

### 8.3 侧边栏组合筛选

- [x] P8-9 侧边栏相册/标签列表实时加载（已在 Phase 3 完成）
- [x] P8-10 当前选中相册/标签高亮（已在 Phase 2 完成）
- [x] P8-11 每个相册/标签后显示数量 Badge（已在 Phase 2 完成）
- [x] P8-12 相册/标签点击不再清空另一个筛选维度，支持组合筛选
- [x] P8-13 星标筛选改为 toggle，不清空相册/标签筛选

### 9.1 搜索增强

- [x] P9-1 搜索框 debounce 300ms
- [x] P9-2 搜索与相册/标签/星标筛选可同时生效

### 9.2 排序

- [x] P9-5 排序下拉：最近导入 / 最早导入 / 标题 A-Z / 标题 Z-A
- [x] P9-6 排序与筛选可同时生效

### 9.3 空状态

- [x] P9-7 全局空状态 UI（引导文案 + "导入第一张图片"按钮）
- [x] P9-8 搜索/筛选无结果时显示"未找到匹配的收藏项"+ 清空筛选按钮

### 9.4 筛选状态管理

- [x] P9-10 "清空筛选"按钮，重置全部筛选条件 + 排序
- [x] P9-11 当有活跃筛选时顶部显示筛选指示器（"当前筛选：星标 + 相册「XX」+ 标签「YY」"）

### 验收

- [x] P8+9-AC 确认：相册可新建并选中、标签输入体验流畅、组合筛选正常、空状态有引导、排序可切换、清空筛选回到全部、构建通过
...
- [x] P10-1 配置应用名称（Figura Vault · AI 手办收藏馆）— tauri.conf.json productName + index.html title
- [x] P10-2 配置应用默认窗口大小 1200×800，最小 900×600，Overlay 标题栏
- [x] P10-3 应用图标（Tauri 默认模板，icns / ico / png 三套齐全）
- [x] P10-4 配置 Tauri 权限（core / dialog / sql / opener / window-dragging）

### 10.2 设置页

- [x] P10-5 创建 `SettingsPage.tsx`（应用信息 + 数据存储 + 图片存储）
- [x] P10-6 展示数据库文件位置（只读）
- [x] P10-7 展示图片存储目录位置（只读）
- [x] P10-8 展示应用版本号 + 技术栈
- [x] P10-9 侧边栏底部"设置"按钮，点击切换到设置页

### 10.3 操作反馈

- [x] P10-10 导入成功后 Toast（"收藏项已导入"）
- [x] P10-11 删除成功后 Toast（"收藏项已删除"）
- [x] P10-12 编辑成功后 Toast（"收藏项已更新"）
- [x] P10-13 星标切换后 Toast（"已添加星标" / "已取消星标"）
- [x] P10-14 操作失败时 Toast（错误信息）
- [x] P10-15 图片加载失败时 FigureCard 显示 fallback 占位（ImageOff 图标 + 提示文字）

### 10.4 代码清理

- [x] P10-16 移除种子数据逻辑（删除 `seed.ts` + `mockFigures.ts`，App.tsx 不再调用 `seedIfEmpty`）
- [x] P10-17 空库开局：首次启动显示空状态引导页

### 10.5 打包与测试

- [ ] P10-18 macOS 打包测试（`tauri build`，.dmg）
- [ ] P10-19 全流程冒烟测试：导入 → 浏览 → 详情 → 滑杆/橡皮擦对比 → 编辑 → 星标 → 筛选 → 删除
- [ ] P10-20 验证数据持久化（关闭重开后数据仍在）
- [-] P10-21 （可选）Windows / Linux 打包测试

### 验收

- [ ] P10-AC 确认：应用可打包安装、全功能可用、数据持久化、操作有反馈提示、空库开局正常 → **v1.0 发布**
